...
public class GetStateInteractorImpl implements GetStateInteractor {
    ...
    @Override
    public void execute(StateListener listener, final OnItemRetrievedListener onItemRetrievedListener) {
            ...
            public void onServiceStateChanged(ServiceState serviceState) {
                switch (serviceState.getState()) {
                    case ServiceState.STATE_IN_SERVICE:
                        ...
                        onItemRetrievedListener.onSuccess(ServiceState.STATE_IN_SERVICE);
                        break;
                    case ServiceState.STATE_OUT_OF_SERVICE:
                        serviceState.setState(ServiceState.STATE_OUT_OF_SERVICE);
                        onItemRetrievedListener.onSuccess(ServiceState.STATE_OUT_OF_SERVICE);
                        break;
                    case ServiceState.STATE_EMERGENCY_ONLY:
                        serviceState.setState(ServiceState.STATE_EMERGENCY_ONLY);
                        onItemRetrievedListener.onSuccess(ServiceState.STATE_EMERGENCY_ONLY);
                        break;
                    case ServiceState.STATE_POWER_OFF:
                        serviceState.setState(ServiceState.STATE_POWER_OFF);
                        onItemRetrievedListener.onSuccess(ServiceState.STATE_POWER_OFF);
                        break;
                    default:
                        onItemRetrievedListener.onError("Error retrieving service state");
                        break;
                }
            }
        };
